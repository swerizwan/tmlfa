...
def window_partition(x, window_size):
    ...
    Returns:
        torch.Tensor: Partitioned windows with shape (num_windows*B, window_size, window_size, C).
    """
    B, H, W, C = x.shape
...
def window_reverse(windows, window_size, H, W):
    """
    Reverse the partitioned windows back to the original tensor.

    Args:
        windows (torch.Tensor): Partitioned windows with shape (num_windows*B, window_size, window_size, C).
        window_size (int): Size of the window.
        H (int): Height of the original tensor.
        W (int): Width of the original tensor.

    Returns:
        torch.Tensor: Reversed tensor with shape (B, H, W, C).
    """
...
class WindowAttention(nn.Module):
    ...
    Args:
        ...
        proj_drop (float, optional): Dropout ratio of results. Defaults to 0.0.
    """
    ...
    def forward(self, x, mask=None):
        ...
        Args:
            x (torch.Tensor): Input tensor with shape (num_windows*B, N, C).
            mask (torch.Tensor, optional): Attention mask with shape (num_windows, Wh*Ww, Wh*Ww). Defaults to None.

        Returns:
            torch.Tensor: Output tensor with shape (num_windows*B, N, C).
        """
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        Returns:
            torch.Tensor: Output tensor with shape (B, H*W, C).
        """
        H, W = self.input_resolution
        B, L, C = x.shape
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        """
        Forward pass of the PatchMerging layer.

        Args:
            x (torch.Tensor): Input tensor with shape (B, H*W, C).

        Returns:
            torch.Tensor: Output tensor with shape (B, H*W/4, 2*C).
        """
        H, W = self.input_resolution
        B, L, C = x.shape
        assert L == H * W, "input feature has wrong size"
...
class BasicLayer(nn.Module):
    ...
    Args:
        ...
        drop (float, optional): Dropout rate. Defaults to 0.0.
        attn_drop (float, optional): Attention dropout rate. Defaults to 0.0.
        drop_path (float | tuple[float], optional): Stochastic depth rate. Defaults to 0.0.
        norm_layer (nn.Module, optional): Normalization layer. Defaults to nn.LayerNorm.
        downsample (nn.Module | None, optional): Downsample layer at the end of the layer. Defaults to None.
    ...
    def forward(self, x):
        ...
        Returns:
            torch.Tensor: Output tensor with shape (B, H*W/4, 2*C) if downsample is used, otherwise (B, H*W, C).
        """
...
class PatchEmbed(nn.Module):
    ...
    Args:
        img_size (int): Image size. Defaults to 224.
        patch_size (int): Patch token size. Defaults to 4.
        in_chans (int): Number of input image channels. Defaults to 3.
        embed_dim (int): Number of linear projection result channels. Defaults to 96.
...
class SwinTransformer(nn.Module):
    ...
    Args:
        img_size (int): Image size. Defaults to 112.
        patch_size (int): Patch token size. Defaults to 2.
        in_chans (int): Number of input image channels. Defaults to 3.
        num_classes (int): Number of classes. Defaults to 1000.
        embed_dim (int): Number of linear projection result channels. Defaults to 96.
        depths (list[int]): Depths of each stage. Defaults to [2, 2, 6, 2].
        num_heads (list[int]): Number of attention heads in each stage. Defaults to [3, 6, 12, 24].
        window_size (int): Local window size. Defaults to 7.
        ...
        qk_scale (float | None): Override default qk scale of head_dim ** -0.5 if set. Defaults to None.
        ...
        ape (bool): If True, add absolute position embedding. Defaults to False.
        patch_norm (bool): If True, add normalization after patch embedding. Defaults to True.
        use_checkpoint (bool): If True, use checkpointing to save memory. Defaults to False.
    """
    ...
    def forward_features(self, x):
        ...
        Returns:
            tuple: Local features, global features, and flattened features.
        """
        patches_resolution = self.patch_embed.patches_resolution
    ...
    def forward(self, x):
        ...
        Returns:
            tuple: Local features, global features, and final output.
        """
